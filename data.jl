...
function next_point(p::Point, ϕ::Float64, isTip::Bool, Si::Float64)
	...
	return (p,Point(p.x + cos(ϕ), p.y +  sin(ϕ)),ϕ)
...
# ╔═╡ 3b66cc12-68bb-11eb-1667-dda4d3bc36fb
getFirst.(next_point.(genk.to,genk.angle,genk.isTip,genk.Si))
...
function step!(fungi::Fungi)
	...
	for i in 1:length(s)
		for j in 1:(length(fungi.to))
			...
			if(intersects(p1,p2,x1,x2))
				not="INTERSECT" * string(p1) * " " string(p2) * " " string(x1) * " " string(x2)
	...
	append!(fungi.isTip, ones(size(s)))
	append!(fungi.Si, ones(Float64,size(s)))
	
	return not
	#append!(fungi.Si, fungi.Si)
	#append!(fungi.from, fungi.to)
	#append!(fungi.to, next_point.(fungi.to))
#	for i in 1:length(fungi.hyphae)
#		# each makes a step of growth
#		push!(fungi.hyphae[i].points,
#				next_point(fungi.hyphae[i].points[length(fungi.hyphae[i].points)],
#						   fungi.hyphae[i].points[length(fungi.hyphae[i].points)-1],
#							fungi))
#		# branch
#		if bernoulli(0.1)
#			push!(fungi.hyphae,
#				  Hypha(vcat(fungi.hyphae[i].points,
#				next_branch(fungi.hyphae[i].points[length(fungi.hyphae[i].points)])),
#						true))
#		# interesect
#		end
#	end
...
intersects(Point(62.3923,60.1962),Point(61.8923,61.0622),Point(0,61.001),Point(100,61.001))
